...
| `services/adapters/conveyor` | Go | 9091 | Conveyor adapter; `POST /tasks` device-task contract. **Emulator mode** (admin-toggled via master-data): simulates CONVEY/DIVERT/MERGE/SCAN with in-memory device state + synthetic telemetry; OFF is the real-hardware seam. |
| `services/adapters/asrs` | Go | 9096 | Shuttle/crane adapter. **Emulator mode**: simulates STORE/RETRIEVE with in-memory state + synthetic telemetry; OFF is the real-hardware seam. |
| `services/adapters/amr-geekplus` | Go | 9093 | Geek+ AMR adapter. **Emulator mode**: simulates TRANSPORT/MOVE with in-memory state + synthetic telemetry; OFF is the real-hardware seam. |
| `services/adapters/autostore` | Go | 9094 | AutoStore grid adapter. **Emulator mode**: simulates BIN_STORE/BIN_RETRIEVE with in-memory state + synthetic telemetry; OFF is the real-hardware seam. |
