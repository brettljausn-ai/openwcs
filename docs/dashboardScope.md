> **Implemented (Dashboards & alerting, including the Phase-4 tail).** The landing dashboard (situation tiles at `/`) and all five menu dashboards (Inbound, Outbound, Replenishment, Stock, ABC movers) are built, along with threshold-based alerting that delivers WARNING/CRITICAL alerts by email and webhook on breach (notification service) and an admin Settings tab for the thresholds. The Phase-4 tail is now built too: **SLA metrics** (on-time-to-cutoff + order-cycle-time median, surfaced as Outbound heroes), a full-screen **andon board** (`/dashboards/andon`), and the ISA-18.2 **alert-system-health** view (`/dashboards/alert-health`). The earlier data gaps are closed as well: receive-errors are now real (no longer a hardcoded 0), ABC risers/fallers are true windowed trailing-14d-vs-90d (a new `sku_pick_daily` daily-tally table, no more EWMA proxy), and dock-to-stock timing is real (received→stored median from `handling_unit.stored_at`). Demo mode now also seeds/clears all of this dashboard sample data. The spec body below is the design doctrine the build follows (ISA-101 / Stephen Few). What remains is only the broader product backlog (real device adapters, real host wire protocols), not anything dashboard-specific. See `docs/AS-BUILT.md` for what each endpoint and screen actually does.
...
* An admin view of alert-system health (alerts per day, chattering alerts, stale alerts) so the alert load itself stays actionable. ISA-18.2 makes measuring the alarm system mandatory in process industries. Now built: `/dashboards/alert-health` over `GET /api/notification/alerts/health`.
