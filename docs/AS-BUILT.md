...
| master-data | 8081 | ✅ | Catalog + outbound config + **hierarchical Areas** (`/api/master-data/areas`: nestable warehouse zones, location `area_id`, recursive subtree → location-ids, resolve) (below). |
...
| adapters/ptl | 9098 | 🟡 | Go; pick-to-light (PTL) device adapter. Implements the uniform device contract (`POST /tasks`) for the gtp service to drive put-lights: `ILLUMINATE` (turn a light on showing a qty + optional colour) and `CLEAR` (turn it off); replies COMPLETED/FAILED synchronously. Health/readiness probes (`/healthz`, `/readyz`) and a `GET /state` view of the currently lit lights (in-memory, lost on restart — enough for the emulator-driven demo and tests). Skeleton device-connection loop (`deviceLoop`) is the real-hardware seam (HTTP/serial PTL controller). |
...
| Schema | Owner | Tables |
|---|---|---|
| `master_data` | master-data | warehouse, attribute_schema, sku, sku_profile, dangerous_goods, unit_of_measure, barcode_type, barcode, handling_unit_type (+ compartments/storable-in-automation/conveyable), equipment, location (+ block/aisle/lane-depth/distance-to-exit + **cell coords side/pos_x/pos_y/pos_z** + **`area_id`** the optional owning area), **`area`** (`V19`, first-class hierarchical area: name/code, warehouse, nullable `parent_area_id` for nesting, archive flag), **storage_block** (+ allowed_hu_types), **shipper**, **warehouse_fulfillment_config**, **shipping_service**, **route**, **label_template** |
...
| `slotting` | slotting | storage_profile, pick_slot, block_policy, putaway_assignment (+ sku_ids for multi-compartment), replenishment_task (+ **`device_task_id`** `V7`, stamped when the task is dispatched as a flow move + status DISPATCHED; + **per-operator reservation columns** `reserved_by`/`reserved_at`/`reserved_by_instance`/`started_at` `V8`, for area-scoped claim-next), reslot_recommendation (+ **`device_task_id`** `V7`, same), sku_velocity (+ velocity offset/processed-event for the auto-ABC EWMA), **sku_pick_daily** (`V6` — per-SKU daily pick tallies, upserted idempotently by the velocity consumer; powers exact trailing-14d-vs-90d ABC risers/fallers) |
| `gtp` | gtp | gtp_station (+ supported_modes + accepting_work + max_in_transit_picking/other, + **pick_layout** + **pick_slots** `V10`), station_node, destination_demand, work_cycle (+ operating_mode, target_hu_id), put_instruction, task_line, **station_queue_entry** |
| `counting` | counting | count_task, count_line (+ station-count state `V4`, `hu_id` `V5`, + **per-operator reservation columns** `reserved_by`/`reserved_at`/`reserved_by_instance`/`started_at` `V6` for area-scoped claim-next), count_schedule |
...
- **Scan resolve** (`/api/master-data/resolve/*`, read-only, RBAC `MASTER_DATA_VIEW`): resolve a
  scanned code to confirm it EXISTS and return the linked graph, for handheld scan verification (the
  process-designer Verify step, see §1 process-designer + `docs/process-designer-spec.md` §4.1). By
  design **never 404**: a miss returns HTTP 200 with
  `found:false`. Reuses the same repositories the SKU/location card reads use.
  - `GET /resolve/sku-by-barcode?warehouseId=&code=` → `{found, ambiguous,
    matchedBarcode{value,uomId,uomCode,type}, sku{skuId,code,description,status},
    ...
    → attribute-schema graph; `ambiguous:true` when the value spans more than one SKU).
  - `GET /resolve/sku?warehouseId=&code=` → same body, resolved by SKU code (`matchedBarcode` null).
  - `GET /resolve/location?warehouseId=&code=` →
...
- **Areas** (first-class hierarchical zoning, `/api/master-data/areas`, see `contracts/openapi/master-data.yaml`):
  a master-data **Area** is a named, nestable region of a warehouse used to scope operator work
  (a level, an aisle range, a zone). Areas form a **tree**: an area can nest inside another via
  `parentAreaId` (`area` table, `V19`), and a **location** now carries an optional `areaId`
  (`location.area_id`), so a location belongs to at most one area while an area's effective scope is
  itself plus every descendant area.
  - `GET /areas?warehouseId=&parentAreaId=` lists areas (top-level when `parentAreaId` is omitted,
    direct children when supplied); `POST /areas`, `GET /areas/{id}`, `PUT /areas/{id}`, and
    `DELETE /areas/{id}` (soft **archive**) are the CRUD surface. Writes are RBAC-gated like the rest of
    master-data CRUD.
  - **Guards**: a parent must be in the **same warehouse** (cross-warehouse parenting rejected) and the
    parent chain may not form a **cycle** (an area cannot be its own ancestor); both are validated on
    create/update.
  - `GET /areas/{id}/location-ids?recursive=true` resolves the area to the set of **location UUIDs** in
    its subtree: a recursive CTE walks the area tree from `{id}` down and returns every location whose
    `area_id` falls anywhere in that subtree (with `recursive=false` it returns only the area's own
    directly-assigned locations). This is the primitive the **area-scoped mobile work** features build on
    (counting line claim, replenishment task claim): an operator picks an area and the service reserves
    only work whose location is inside that subtree.
  - `GET /areas/resolve?warehouseId=&ref=` resolves an area by code (or other reference) to
    `{found, areaId, areaCode, ...}` for handheld scan/verify, by design a 200 `found:false` on a miss
    (never 404), used by the process-designer `area` verify kind.
  - **Operators get their area from an in-process picker/scan**, not a stored binding: there is **no
    persistent operator → area assignment** in the system. The chosen `areaId` lives in the handheld
    process data object for the duration of the run.
  - **Frontend**: master-data gains an **Areas** tab with an `AreaDialog` for creating/editing nested
    areas (the parent select excludes the area itself and its descendants to keep the tree acyclic), and
    the **Locations** screen gains an **Area** select so a location can be filed under an area.
...
#### 6a-i. Fulfilment integrity: a correct pick (reservation consume + negative-stock safety + audit)

The pick confirm is now a **correct fulfilment mechanism**, not just a stock decrement. `POST
/api/orders/pick-tasks/{lineId}/confirm` (and the curated `picking.book` task that calls it) does four
things on top of the previous behaviour:

- **Available-at-location guard (never drive stock negative)**: before posting, order-management reads
  the pickable quantity at the pick face via the new inventory client
  (`GET /api/inventory/availability?warehouseId=&skuId=&locationId=`, available-to-promise =
  on-hand minus reserved at that location) and **clamps the booked qty to what is on hand**. If the
  operator confirmed more than is available there, only the available qty is booked and the line is
  marked **`SHORT`** (a zero-available face books nothing). The read is **best-effort**: a read failure
  or an unknown location falls back to the requested qty (the inventory projection still has its own
  non-negative DB check + underflow throw as the hard backstop, §4). So a pick can never drive stock
  negative.
- **Reservation consume (keeps ATP honest)**: the `Picked` event now carries the order line's
  `reservationId`. The inventory projection's Picked handler **partial-consumes that named reservation**
  in the same idempotent (`processed_event`) apply transaction as the stock decrement: it reduces
  `Reservation.qty` and marks it `CONSUMED` when fully drawn down (`sumHeld` already excludes
  `CONSUMED`). Without this, consuming stock while a HELD reservation still stands would double-count
  the pick against available-to-promise (`ATP = onHand − reserved`). A null/missing/already-consumed
  reservation is a no-op (legacy `Picked` events and replays just decrement, as before).
- **Operator + process + order-type stamped on the transaction**: the PICK `order_line_transaction`
  now records `process_instance_id` and `order_type` (migration **`V12`**, both nullable for
  back-compat) alongside the existing `actor`. `confirmPick` accepts an optional `processInstanceId`
  (request body field **or** `X-Process-Instance` header), so a pick driven by a handheld process is
  traceable back to the run and the order kind without a join.
- **Richer Picked event payload**: the `Picked` event now carries `reservationId`, `orderType`,
  `processInstanceId` and `actor` in addition to the existing `warehouseId`/`skuId`/`locationId`/
  `uomCode`/`qty`, so downstream consumers can consume the reservation and trace the pick to a user.
- **Config**: the new order-management inventory client base url is
  `openwcs.order-management.inventory-base-url` / `OPENWCS_INVENTORY_BASE_URL` (default
  `http://localhost:8082`).

The **`StockAdjusted`** (Adjust) payload also gained an optional **`reason`** field for audit
(consumed by the new `inventory.reduce` exception task, §7i). The inventory non-negative stock guard
(DB check + projection underflow throw) is unchanged; no inventory migration was needed.
...
- **Conveyor adapter** (`services/adapters/conveyor`, Go): `POST /tasks` simulates a move,
  ...
  commands (→ FAILED).
- **Pick-to-light (PTL) adapter** (`services/adapters/ptl`, Go, port 9098, family `PTL`): a real
  device adapter (not a FAILED stub) driven by the **gtp** service, not by flow-orchestrator. `POST
  /tasks` accepts `ILLUMINATE` (light id + qty + optional colour) and `CLEAR` (→ COMPLETED), any
  other command → FAILED. It tracks the lit set in memory and exposes it at `GET /state` (plus
  `/healthz`, `/readyz`); its `deviceLoop` is the seam for a real HTTP/serial PTL controller. The
  gtp `HttpLightControlClient` targets it (or the equipment-emulator's PTL family when the
  hardware-emulator flag is ON), best-effort so a light failure never blocks the pick — see §7e.
- **Hardware emulator mode**: a dedicated `equipment-emulator` service (port 9097) simulates all
  device families. When `HARDWARE_EMULATOR_ENABLED` is ON, flow-orchestrator routes device
  tasks to it instead of the real adapters; the service accepts CONVEY/DIVERT/MERGE/SCAN,
  STORE/RETRIEVE, TRANSPORT/MOVE, and BIN_STORE/BIN_RETRIEVE, plus a **PTL** family
  (ILLUMINATE/CLEAR, the gtp service points its light client here when the flag is ON), maintaining
  **in-memory device state** — never opening a hardware connection. Each command sleeps a realistic per-family/command
...
**Configurable picking layout** (`V10__pick_layout.sql`): a station also carries a `pick_layout`
(`gtp_station.pick_layout`, default `ONE_TO_ONE`, CHECK-constrained) and an optional `pick_slots`
(`gtp_station.pick_slots`, the slot count for `ONE_TO_N`). The layout is independent of `mode`: it
selects how the operator console renders the target side and how pick-to-light is driven.
- `ONE_TO_ONE` — one stock HU serves one destination at a time (the classic single-carton GTP).
- `ONE_TO_N` — one stock HU fans out to `N` lit pick slots (a batch station); `pick_slots` is `N`,
  validated `>= 2`.
- `PUT_WALL` — a rack of lit cubbies; only valid when the station `mode` is `PUT_WALL`.
Validation lives in the station config service: `ONE_TO_N` requires `pickSlots >= 2`; `PUT_WALL`
requires `mode = PUT_WALL` (existing put-wall stations are migrated to `pick_layout = PUT_WALL`).
`pickLayout` + `pickSlots` are surfaced on the `StationView`, the workplace view and the `WorkCycle`
view so the operator console can render by layout. The layout is set in the GTP config screen (a
"Picking layout" selector plus a "Pick slots" input shown for `ONE_TO_N`); put-wall cubbies and
put-light ids continue to be configured per ORDER node as before.
...
**PICKING console, by layout** (`ui/src/gtpops/GtpOpsScreen.tsx`, CSS `op-gtp-*` in
`ui/src/theme/app.css`): while an operator is actively picking, the active pick is one bordered box
split into three columns. **Left** = the source (the active stock tote + its SKU, unchanged).
**Middle** = the take quantity (`→ N →`, the sum of OPEN puts for this present). **Right** branches
on the cycle's `pickLayout`:
- `ONE_TO_ONE` — a single focused quantity input (autofocused, defaulting to the required qty):
  Enter or Confirm books a full pick, a lower value books a short pick (clamped, no negatives);
  focus returns after each confirm and as the next put becomes active.
- `ONE_TO_N` — all `N` slots rendered left-to-right (`SlotsView`, wrapping). Slot identities are the
  ORDER nodes by position, else a synthetic `1..N` from `pickSlots`. States: GREY (no put targets it
  this pick), ORANGE (an OPEN put targets it, showing its qty), GREEN (the put is CONFIRMED).
  Tapping an ORANGE slot confirms its full qty; a small inline stepper confirms a short qty.
- `PUT_WALL` — only the needed put lights are shown (orange until picked, green once confirmed); tap
  a light to confirm, with the same short-stepper for a partial put.
Auto-release on completion stays. The backend calls (confirm put / close cycle) are unchanged.

**Pick-to-light driving** (`LightControlClient` / `HttpLightControlClient`): the gtp service now
physically drives put-lights as a side effect of the pick cycle. On present it issues an
`ILLUMINATE` (with the light id + qty) for each put light; on confirm and on close it issues a
`CLEAR`. Every call is **best-effort** — short (2 s) timeouts and any failure is logged at WARN and
swallowed, so a light fault never blocks the pick. The target base URL is resolved like
flow-orchestrator dispatch: the **equipment-emulator** when the hardware-emulator flag is ON,
otherwise the real **PTL adapter** (`openwcs.gtp.ptl-base-url`, default `:9098`;
`openwcs.gtp.emulator-url`, default `:9097`). Both speak the uniform device contract
(`POST /tasks`, `family:"PTL"`, command `ILLUMINATE` / `CLEAR`). See §7b-PTL.
...
## 7i. Process-designer RF resilience (exact resume, RF-users, replay, fulfilment tasks)

The configurable handheld process designer (service row above) gained a **mobile-resilience** layer so
an operator's flow survives a refresh, a device switch, or an RF re-login, and a supervisor can see and
move in-progress work and replay exactly what an operator saw.

- **Per-screen step history (exact resume + replay)**: a new append-only `process_step_event` table
  (`instance_id`, `seq`, `step_id`, `step_type`, `data`, `entered_by`, `at`; unique on
  `(instance_id, seq)`) and an `assigned_to` column on `process_instance` (backfilled from
  `started_by`); migration **`V6`**. The mobile client posts **every screen advance** to
  `POST /api/process-designer/instances/{id}/step {seq, stepId, stepType, data}` (idempotent on
  `instanceId+seq`): it merges the data and sets the **exact `current_step`**, so the server always
  holds the precise spot the operator is at. A task-step `checkpoint` also appends a step event.
- **Exact resume**: `GET /api/process-designer/instances/{id}` resumes at the exact recorded step with
  the merged data, so a refresh / new device / re-login continues on the same screen rather than from
  the last task checkpoint.
- **Step replay log**: `GET /api/process-designer/instances/{id}/steps` returns the ordered step
  history (oldest first) for replay.
- **Assignment + reassign**: `assigned_to` is who currently owns the running instance (the immutable
  `started_by` stays as the create audit). `GET /api/process-designer/instances` accepts an
  `assignedTo` filter; `assignedTo` is on `InstanceSummary` + `InstanceView`. `POST
  /api/process-designer/instances/{id}/reassign {toUser}` (supervisor-gated, `PROCESS_DESIGN_EDIT`)
  moves the work to another operator, keeps `started_by`, and writes an audit step event.
- **Username on every transaction**: the engine now injects the operator username (`actor`) into task
  inputs (alongside the `warehouseId` + `instanceId` from the area-scoped epic), so the operator's
  username is persisted on every resulting transaction.
- **Three fulfilment tasks** (curated, identity-forwarded, real audited endpoints):
  - **`picking.book`**: book picked stock from a location to an order line. Inputs `lineId`,
    `locationId`, `pickedQty`, optional `short`; auto-injects the running `instanceId` as
    `processInstanceId`; calls `POST /api/orders/pick-tasks/{lineId}/confirm`. Outputs `bookedQty`
    (what order-management actually booked, clamped to available, §6a-i), `requestedQty`, `pickStatus`.
  - **`inventory.reduce`**: exception correction that reduces stock at a location for a stated reason
    (damage, write-off). Inputs `skuId`, `locationId`, `qty`, `reason`, optional `huId`/`batchId`;
    `warehouseId` auto-injected. **Pre-checks availability** at the location and **rejects an
    over-reduction** before posting (never drives stock negative / never emits a poison event), then
    posts a **negative `StockAdjusted`** carrying `reason` + `actor` + `processInstanceId`. Output
    `eventId`.
  - **`order.load`**: load an order from the operator's scanned/typed order code. Input `orderCode`;
    `warehouseId` auto-injected; calls `GET /api/orders/resolve`. Outputs `found`, `orderId`,
    `orderRef`, `orderType`, `orderStatus`, `lineCount` (then branch on `found` / `orderStatus`).

### 7i-front. Mobile runtime + supervisor RF screens (frontend)

- **Durable step queue**: the mobile runtime persists every screen advance to the server via a durable
  **offline queue** (`stepQueue`, IndexedDB with a localStorage fallback, in the same `owcs-process`
  DB as the checkpoint queue) and **advances the operator immediately** (offline-first; it never blocks
  on a screen advance, unlike a task checkpoint which holds). So a **refresh / device switch / RF
  re-login resumes on the EXACT step with the data intact**.
- **Resume launcher**: the handheld **OperatorMenu** has a **"Resume in-progress work"** launcher
  listing the operator's own RUNNING instances (server-filtered on `assignedTo` + `status`), so a
  re-logged-in operator (or a different device) picks the run back up.
- **RF-Users screen** (supervisor, `process:rf-users`, Engineering, `/rf-users`, ADMIN/SUPERVISOR):
  lists the **active operators** on configurable handheld processes (derived from the RUNNING instances
  grouped by `assignedTo`) with the process and the current workstep, plus a **Reassign** action.
- **Replay viewer**: steps through what each operator saw and entered, **reusing the real screen
  renderer** (`ProcessScreenView`) over the instance's step history.
...
- **Demo-box data snapshot (fixtures)** `ui/src/demo/fixtures/*.json`: GET responses snapshotted from
  ...
  The demo is honest sample data, not a claim of full data on every screen. The **dashboard and
  report fixtures carry a fuller curated dataset** (deterministically generated, ids referencing the
  master-data fixtures so they resolve to friendly names): orders dashboard hour-of-day curve and
  today figures, a 14-day SLA series, four dispatch routes, replenishment projected stockouts, ABC
  Pareto over the demo SKUs, 40-row stock-by-SKU, 14-day storage-density, multi-day scan-quality /
  traffic / transit-times / storage- and device-movements, 90-day inbound/outbound order flow, and a
  14-day alert-health series, so the Dashboards and Reporting areas look populated rather than sparse.
- **In-memory pick engine** `ui/src/demo/pickEngine.ts` (the only "live" state): on demo start it arms
  ...
  **dirty-tote / broken-units exceptions**). A `reset()` re-arms all 5; no timers or state survive a
  reload.
- **Hardware visualisation (the live 3D twin), now driven in the demo** `ui/src/demo/demoTwin.ts`:
  the existing **Hardware visualisation** screen (`ui/src/hardwaretwin/`) renders a live 3D twin from
  the automation **topology** (geometry) plus the flow **device-task** feed and the **twin read-models**
  (tote paths / AMR fleet / ASRS cranes). The demo has no backend, so `demoTwin.ts` fabricates the SAME
  shapes from the wall clock: a fixed layout (a recirculating **conveyor loop** around an **ASRS rack**,
  a **sorter** and two **pick stations PP1/PP2**, single-sourced so the drawn belt and the tote motion
  cannot drift) plus pure functions of `nowMs` that emit RUNNING CONVEY device tasks per tote, each
  tote's resolved conveyor polyline (server-stamped waypoints the twin replays ~5 s behind real time,
  motion.ts), three roaming **AMRs** and one **ASRS crane** working the rack. The result: **totes drive
  around the loop in real time**, the rack lights up and the crane lifts, with no backend. Wired into
  `mockApi.ts`: `/api/master-data/equipment` (the equipment library so the renderer classifies
...
- **The public page + serving.** `/live-demo` (template `public/views/pages/live-demo.ejs`, route in
  ...
  **Live demo** now points to the in-site sandbox, with a secondary **Full demo box** link to
  ...
  also highlights **Operations → Hardware visualisation** (the live 3D twin with totes driving round
  the loop, `ld.guideTwinNote`).
