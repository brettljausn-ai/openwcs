...
| counting | 8095 | ✅ | Cycle / stock counting: count tasks (scope LOCATION/SKU/ZONE/BLOCK, BLIND vs VARIANCE), ABC-cadence schedule generator, capture counts → variance vs an inventory-expected snapshot → within-tolerance auto-approve (posts a `StockAdjusted` event) or out-of-tolerance recount; blind hides expected/variance. Seams: GTP STOCK_COUNT station + cycle-count BPMN (by id), adjustment via txlog. **Demo quick-seed** (demo-mode only): `POST /api/counting/demo/seed {warehouseId, count?:10}` bulk-builds sample count tasks from the seeded DEMO catalog, returns `{created}`, guarded to demo mode ON; drives the "Add 10 count tasks" button shown on the stock-counting screen only when demo mode is on. |
