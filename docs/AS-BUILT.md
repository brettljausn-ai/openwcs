...
- `POST /api/gtp/queue/{entryId}/complete` — operator completion; **now fans out to `POST /api/flow/induction/entries/{id}/done` (flow marks the entry DONE) then triggers gtp store-back** (`flow.createTransport(... "STORE" ...)`).
...
`/api/master-data/locations/{id}` → `/api/master-data/storage-blocks/{id}`),
