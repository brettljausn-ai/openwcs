...
- `POST /api/gtp/stations/{id}/queue {huId, huCode, skuId, skuCode, qty, mode, family, distanceM, countTaskId?, countLineId?}` —
...
**Count-tote routing (counting service):** when the hardware emulator is ON and a count task
is created for cells in an automated storage block (`SHUTTLE_ASRS`, `CRANE_ASRS`, `AUTOSTORE`,
`AMR_GTP`), the counting service calls the GTP queue endpoint to pin the tote to an active
`STOCK_COUNT` station and calls flow-orchestrator to create the retrieval transport dispatched to
the adapter family that services the cell (`ASRS`, `AUTOSTORE`, or `AMR`) — not a hardcoded
`ASRS`. **GTP store-back family resolution:** `POST /api/gtp/queue/{entryId}/complete` triggers
a store-back; the STORE transport's adapter family is resolved from the destination location's
storage type via `MasterDataClient` (two calls: `/api/master-data/locations/{id}` → `/api/master-data/storage-blocks/{id}`),
with `"ASRS"` as fallback; config `OPENWCS_MASTER_DATA_BASE_URL`
(property `openwcs.gtp.master-data-base-url`, default `http://localhost:8081`). This is the
first end-to-end multi-family wiring of the counting→GTP seam (emulator mode only).
...
- **GTP station execution** (`gtp`, ADR 0006): built — STOCK + ORDER/PUT_WALL nodes, batch
  ...
  **Topology-projected station nodes** (`POST /stations/{id}/nodes/sync`, `inboundDistanceM`): the
  ...
  wired to inventory. Count-tote routing to the station queue (for all automated storage families — ASRS, AutoStore, AMR) is wired via the counting service (emulator mode only); GTP store-back also resolves the STORE transport's adapter family from the destination's storage type.
