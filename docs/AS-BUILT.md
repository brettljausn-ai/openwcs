...
_Last updated: 2026-06-15 (execution layer: three features land the move-execution slice. (1) **Physical move dispatch** — flow `POST /api/flow/moves` dispatches a RELOCATE / AutoStore BIN_RELOCATE device task to physically move an HU and books the new inventory location on completion (the `HandlingUnitMoved` audit fires; stock follows the HU, so no `StockMoved` for HU-bound moves = no double-apply); slotting gains a `FlowClient` + three dispatch endpoints (reslot / replenishment / decant-putaway) that flip plans to DISPATCHED (`V7` `device_task_id`); inventory adds `GET /handling-units/{id}/contents` and proves the dormant `StockMoved`/`PutawayCompleted` projection for loose stock. (2) **Picking execution UI** — order-management adds an operator pick queue + per-line confirm/short, and the UI adds a guided RF-style Picking screen at `/picking`. (3) **Twin live views** — equipment-emulator exposes AMR-fleet + AutoStore-grid telemetry, flow maps it to world coords (`/api/flow/twin/amr-fleet`, `/api/flow/twin/autostore`), and the hardware twin renders the AMR fleet + AutoStore ports + fill %. Earlier (AI assistant): a new `assistant` service (8096) runs an Anthropic Claude Messages-API tool-use agentic loop over read-only, identity-forwarded WCS endpoints, answering only from tool results; the Anthropic key + model + enabled flag live in master-data `system_configuration`, write-only on the public API and read by the assistant from a network-only internal endpoint; a global floating chat widget + a Settings → AI Assistant admin tab; requires an admin-supplied Anthropic API key. Earlier: dashboards & alerting: a landing situation dashboard + five menu dashboards over new read-only per-service aggregation endpoints, the notification service is now functional (threshold evaluator with email/webhook alert delivery), and admin alert-threshold settings; self-service password change at login rescues "not fully set up" accounts; per-screen off/read/write access with gateway enforcement via ScreenWriteCatalog; database console governed by admin-database screen access and grantable to non-admin roles; multilanguage UI ships in 5 languages out of the box)_
...
- **Pick queue** `GET /api/orders/pick-tasks?warehouseId=` (`ORDER_VIEW`): the operator work queue,
  ...
  Each entry carries the SKU, the qty to pick, the order ref, and (when known) the pick `locationId`.
...
- **Front-end** the guided RF-style **Picking** screen (`/picking`, Operations) drives this: one big
  next-pick card (location code, SKU code + name + image, qty), **Confirm** / **Short**, an advancing
  queue, codes resolved via `useCatalog` (no UUIDs), Enter = confirm. Pick-by-light, pick-by-voice and
  RF-gun guidance are hardware seams layered on this screen (not yet built).
- **Gap**: the allocated pick location is not yet threaded from allocation onto the order line, so the
  queue's `locationId` can be null until allocation persists it (documented follow-up).
...
- **flow** `POST /api/flow/moves {warehouseId, huId, huCode, fromLocationId, toLocationId, family, reason}`
  (`DEVICE_OPERATE`) dispatches a **RELOCATE** device task (AutoStore family → **BIN_RELOCATE**) to
  physically move an HU. On the completion callback flow books the HU's new location in inventory and
  the existing **`HandlingUnitMoved`** txlog audit fires (§7b). Because stock follows the HU via the
  registry booking, **no `StockMoved` is emitted** for the HU-bound move (the projection does not
  double-apply, §4). The single-leg RELOCATE move is built; a multi-leg RETRIEVE→CONVEY→STORE move is
  a follow-up.
...
reason}` it dispatches a **RELOCATE** device task (family ASRS; AutoStore → **BIN_RELOCATE**) to
move an HU between two locations. On the completion callback flow books the HU's new inventory
location (`PUT /api/inventory/handling-units/{id}/location`) and the terminal-transition
`HandlingUnitMoved` audit (above) fires; stock follows the HU via the registry booking, so no
`StockMoved` is projected (no double-apply, §4). Slotting's `reslot/.../dispatch`,
`replenishment/.../dispatch` and `decant/putaway` endpoints all funnel into this. The single-leg
RELOCATE is built; a multi-leg RETRIEVE→CONVEY→STORE move is a follow-up.

**Twin AMR/AutoStore telemetry** (`GET /api/flow/twin/amr-fleet?warehouseId=`,
`GET /api/flow/twin/autostore?warehouseId=`, DEVICE_VIEW; `TwinTelemetryService`): twin read models
for the Hardware visualisation page that complement the conveyor tote-paths. They poll the
`equipment-emulator`'s `GET /amr/fleet` (simulated AMR robot positions / status / carried HU) and
`GET /autostore/grid` (grid fill + per-port activity), map the simulated state to the warehouse's
world coordinates, and serve it to the 3D scene. Best-effort: when the emulator is off (or no
telemetry) the endpoints return empty and the scene simply renders nothing for those families. The
emulator's AMR + AutoStore task processing updates the telemetry, so the views animate against real
device-task load.
