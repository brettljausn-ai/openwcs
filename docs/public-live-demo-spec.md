...
The Hardware visualisation screen is NOT snapshotted: its topology + live feeds are generated by the
`demoTwin.ts` simulator (§5.7a), not captured fixtures.
...
### 5.7a Hardware visualisation (live 3D twin)
`ui/src/demo/demoTwin.ts` drives the **Hardware visualisation** screen (`ui/src/hardwaretwin/`) without
a backend. That screen builds a live 3D twin from data it already polls: the automation **topology**
(geometry, loaded once) plus the flow **device-task** feed and the **twin read-models** (tote paths /
AMR fleet / ASRS cranes). The simulator fabricates the SAME shapes:
- A fixed layout single-sourced here so the drawn belt and the tote motion cannot drift: a
  recirculating **conveyor loop** around an **ASRS rack**, a **sorter** on the bottom run, and two
  **pick stations** linked to the real PP1/PP2 station ids. Returned as the equipment library
  (`/api/master-data/equipment`, so the renderer classifies conveyor/asrs/sorter) and the topology
  (`/api/flow/automation/topology`).
- Pure functions of `nowMs`: `deviceTasks` emits a RUNNING `CONVEY` task per tote (so the twin renders
  it in transit) plus a few ASRS cycles and recent completions; `totePaths` emits each tote's resolved
  conveyor polyline as server-stamped waypoints the twin replays ~5 s behind real time (the existing
  `motion.ts` interpolation buffer); `amrFleet` and `asrsCranes` move three robots and a crane (the
  twin eases their pose per frame). Wired into `mockApi.ts` at `/api/flow/device-tasks` (filter-aware),
  `/api/flow/twin/tote-paths`, `/api/flow/twin/amr-fleet`, `/api/flow/twin/asrs-cranes`.
- Net effect: totes drive round the loop in real time, the rack lights up and the crane lifts, reusing
  the production twin renderer (no second, cruder scene). All in JS memory; a reload re-seeds.
