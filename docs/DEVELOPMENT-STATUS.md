...
| ui | React/TS | 5173 dev / 80 prod | 🟡 | React/Vite SPA: **Keycloak login** + sidebar **app shell** + **dashboard** + **screen permission catalog** (role/user-gated nav & routes). Built screens: topology (React Flow), BPMN designer (bpmn-js), slotting. Reserved/stub (agents building): inbound, outbound, counting, GTP ops + config, transport, stock transactions, master data, settings, users, access control. Dockerised (nginx) on host **:80** under compose `--profile apps` (proxies `/api`→gateway, `/realms`+`/admin`→Keycloak); **gateway JWT enforced**. |
