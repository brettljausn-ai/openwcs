...
| counting | `DemoSeedTest`, `CountRoutingTest` | Testcontainers + mocked clients (**demo seed** creates sample count tasks over demo stock when demo mode on, rejected when no stock; **ASRS routing**: emulator ON + ASRS cell + active station → flow transport created + tote enqueued in STOCK_COUNT mode; emulator OFF → no routing; routing failure → count task still created) |
