...
| **4 — Counting & operations** | 🟡 | `StockAdjusted` projection ✅; **cycle/stock counting service ✅** (`counting`: count tasks, ABC-cadence schedule, blind/variance, recount + reconciliation → `StockAdjusted`); **ASRS count-tote routing to GTP counting station ✅** (emulator mode: counting service routes ASRS totes to an active STOCK_COUNT station + creates flow transport task; best-effort); dashboards/alerting ⬜; **at-station blind count UI ✅** (GTP ops `CountPanel` — GTP-routed totes only); general operator count capture UI ⬜. |
