...
// state.go holds the in-memory simulated device state and telemetry for the AMR Geek+ adapter.
...
// deviceState is the simulated AMR fleet's running tallies.
type deviceState struct {
	...
	moves   int64
	scanned int64
	// robotX/robotY is an example position for one robot, walked on each MOVE/TRANSPORT.
	robotX int64
	robotY int64
...
func (s *deviceState) snapshot() map[string]interface{} {
	...
	return map[string]interface{}{
		...
		"devices": map[string]interface{}{
			...
			"moves":         s.moves,
			"scanned":       s.scanned,
			"robotPosition": map[string]int64{"x": s.robotX, "y": s.robotY},
		},
