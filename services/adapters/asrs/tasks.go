...
// tasks.go implements the uniform device contract POST /tasks for the ASRS adapter.
...
// supportedCommands are the moves this ASRS adapter accepts.
var supportedCommands = map[string]bool{"STORE": true, "RETRIEVE": true, "SCAN": true}
...
// live hardware is connected (the real protocol client is unimplemented). Hardware emulation moved
...
func handleTask(w http.ResponseWriter, r *http.Request) {
	...
	// No real-hardware code exists yet (the protocol client is the TODO in main.go's deviceLoop), and
	...
	log.Printf("%s: WARNING task %s refused: %s %s for %s (equipment %s) not executed, no live %s hardware is connected (protocol client unimplemented; tasks reach this adapter only while the emulator flag is OFF); task FAILED, turn the emulator ON or connect real hardware",
