...
/**
 * Per-endpoint RBAC for gtp (build.md §4.8). Reads require ORDER_VIEW; presenting stock and
 * confirming puts (station operation) require DEVICE_OPERATE — the permission held by the
 * OPERATOR role. Gated by {@code openwcs.security.enabled}.
 */
@Component
public class RbacFilter extends OncePerRequestFilter {
    ...
    private Permission requiredPermission(HttpServletRequest request) {
        ...
        if (!enabled || !uri.startsWith("/api/gtp")) {
            return null;
        }
        if (HttpMethod.GET.matches(request.getMethod())) {
            return Permission.ORDER_VIEW;
        }
        return Permission.DEVICE_OPERATE;
    }
}
