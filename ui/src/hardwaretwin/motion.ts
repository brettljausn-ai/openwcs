...
/** Project a world point onto a path: arc-length station `s` of the nearest point plus distance `d`. */
export function projectPointOnPath(path: PathSample, p: XZ): { s: number; d: number } {
  return projectOnto({ pts: path.pts, cum: path.cum, total: path.total, closed: false }, p)
}
